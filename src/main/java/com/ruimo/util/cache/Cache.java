...
<#if locale="ja">
    assertEquals(new Integer(55), cache.getValue()); // computeValue()が呼び出されない。
<#else>
    assertEquals(new Integer(55), cache.getValue()); // computeValue() will not be invoked.
</#if>
</pre>
 */
abstract public class Cache extends AbstractCache {
     ...
     */
    abstract protected Object computeValue();
